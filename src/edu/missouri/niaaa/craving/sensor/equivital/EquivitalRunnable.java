...
public class EquivitalRunnable implements Runnable, ISemDeviceSummaryEvents,
	...
	// List to store the chest accelerometer data in order to do compression
	// later.
	...
	protected EquivitalRunnable(String address, String name, String ID) {
		...
		try {
			...
			log.d("ERROR:License Code and Developer Name don't match");
			return;
		}
	}
	...
	private void connectToDevice(String address) {
		...
		ISemConnection connection = SemBluetoothConnection
	...
	public void summaryDataUpdated(SemDevice arg0, SemSummaryDataEventArgs arg1) {
		// TODO Auto-generated method stub
		updateSummary(arg1.getSummary().getMotion().name(), arg1.getSummary()
				.getOrientation().name(), arg1.getSummary().getBreathingRate()
				.getBeltSensorRate(), arg1.getSummary().getBreathingRate()
				.getEcgDerivedRate(), arg1.getSummary().getBreathingRate()
				.getImpedanceRate(), arg1.getSummary().getHeartRate()
				.getEcgRate(), arg1.getSummary().getQualityConfidence()
				.getBeltQuality(), arg1.getSummary().getQualityConfidence()
				.getECGQuality(), arg1.getSummary().getQualityConfidence()
	...
	// private void updateAcceleormeterSummary(double resultantAcc, double
	// lateralAcc, double longitudinalAcc, double verticalAcc) {
	private void updateAcceleormeterSummary(double resultantAcc) {
		// TODO Auto-generated method stub
		// String
		// AccelerometerDataFromChestSensor=String.valueOf(resultantAcc)+","+String.valueOf(lateralAcc)+","+String.valueOf(longitudinalAcc)+","+String.valueOf(verticalAcc);
		if (compressChestSensorAccelerometerData(resultantAcc)) {
			...
			// if
			// (medianFilterChestSensorAccelerometerData(averageAcc,delaySecond)){
			String AccelerometerDataFromChestSensor = String
					.valueOf(averageAcc);
			// String AccelerometerDataFromChestSensorMedian =
			// String.valueOf(medianFilteredAcc);
			// String AccelerometerDataFromChestSensorDiff =
			// String.valueOf(medianFilteredAcc-averageAcc);
	...
	Handler chestSensorDataHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			writeChestSensorDatatoCSV(String.valueOf(resBundle
					.getString("DATA")));
	...
	Handler chestSensorAccDataHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			// {
			Bundle resBundle = (Bundle) msg.obj;
			writeChestSensorAccelerometerDatatoCSV(
					String.valueOf(resBundle.getString("ACC")),
					"chestAccelerometer");
	...
	private void writeChestSensorDatatoCSV(String chestSensorData) {
		...
		/*
		 * // Deal with the delay when applying the filter int delayMillis =
		 * 1000*(delaySecond+1); cal.setTimeInMillis(cal.getTime().getTime() +
		 * delayMillis);
		 */
		String dataToWrite = String.valueOf(cal.getTime()) + ","
				+ chestSensorData;
		...
		if (dataPoints.size() == 57) {
			...
			transmitData.execute("chestsensor" + "." + phoneID + "."
					+ deviceName + "." + dateObj, enformattedData);

			log.d("Chest Summary Data Point Sent " + enformattedData);
			subList.clear();
			subList = null;
		}
	...
	private void writeChestSensorAccelerometerDatatoCSV(
			String chestSensorAccelerometerData, String fileName) {
		// TODO Auto-generated method stub
		// Toast.makeText(serviceContext,"Trying to write to the file",Toast.LENGTH_LONG).show();
		...
		if (AccDataPoints.size() == 57) {
			...
			transmitData.execute(fileName + "." + phoneID + "." + deviceName
					+ "." + dateObj, enformattedData);
			log.d("AVG Accelerometer Data Point Sent");
			subList.clear();
			subList = null;
		}
		/*
		 ...
		 *
		 * if(AccDataMedianPoints.size()==57) { List<String> subList =
		 * AccDataMedianPoints.subList(0,56); String data=subList.toString();
		 * String formattedData=data.replaceAll("[\\[\\]]","");
		 * //sendDatatoServer
		 * ("chestsensor"+"."+phoneAddress+"."+deviceName+"."+dateObj
		 * ,formattedData); TransmitData transmitData=new TransmitData();
		 * transmitData
		 * .execute(fileName+"."+phoneAddress+"."+deviceName+"."+dateObj
		 * ,formattedData);
		 * Log.d("Equivital","Median Accelerometer Data Point Sent");
		 * subList.clear(); subList=null; } } else if
		 * (fileName.equals("chestAccelerometerDiff")){
		 * AccDataDiffPoints.add(dataToWrite+";");
		 *
		 * if(AccDataDiffPoints.size()==57) { List<String> subList =
		 * AccDataDiffPoints.subList(0,56); String data=subList.toString();
		 * String formattedData=data.replaceAll("[\\[\\]]","");
		 * //sendDatatoServer
		 * ("chestsensor"+"."+phoneAddress+"."+deviceName+"."+dateObj
		 * ,formattedData); TransmitData transmitData=new TransmitData();
	...
	private Boolean compressChestSensorAccelerometerData(
			Double rawChestSensorAccelerometerDataMilli_Gs) {
		...
		if (chestAccList.size() <= 25) {
			...
			/*
			 * //median part Collections.sort(chestAccList); int medianPosition
			 * = chestAccList.size()/2; if (chestAccList.size()%2==0){
			 * medianFilteredAcc =
			 * (chestAccList.get(medianPosition)+chestAccList
			 * .get(medianPosition-1))/2; } else { medianFilteredAcc =
			 * chestAccList.get(medianPosition); }
			 */
			chestAccList.clear();
			chestAccList.add(chestSensorAccelerometerData);
			return true;
	...
	/**
	 * @author Ricky 2/6 Funtion to apply the median filter.
	 * @param compressedAVGChestSensorAccelerometerData
	 *            : Avg value of 1 second data
	 * @param filterSize
	 *            : filter shifting window size (second)
	 */
	/*
	 * private Boolean medianFilterChestSensorAccelerometerData(Double
	 * compressedAVGChestSensorAccelerometerData, int filterSize){ if
	 * (medianWindowQueue.size()<=filterSize){
	 * medianWindowQueue.add(compressedAVGChestSensorAccelerometerData); return
	 * false; } else { medianList = new ArrayList<Double>(medianWindowQueue);
	 * Collections.sort(medianList); int medianPosition = medianList.size()/2;
	 * if (medianList.size()%2==0){ medianFilteredAcc =
	 * (medianList.get(medianPosition)+medianList.get(medianPosition-1))/2; }
	 * else { medianFilteredAcc = medianList.get(medianPosition); }
	 * medianList.clear(); medianWindowQueue.poll();
	 * medianWindowQueue.add(compressedAVGChestSensorAccelerometerData); return
	 * true; }
	 *
	 *
